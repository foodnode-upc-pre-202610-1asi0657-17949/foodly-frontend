  ...
  <div class="menu-page">
    ...
    <main class="menu-main">
      ...
      <div class="menu-header">
        ...
        <button v-if="products.length" class="btn-new-menu" @click="goEdit">
      ...
      <div v-else-if="!loading && products.length === 0" class="empty-state">
        ...
        <h2 class="empty-state__title">Todavía no tienes platos en tu menú</h2>
        <p class="empty-state__sub">Agrega tus primeros platos para que los comensales puedan verlos en el radar.</p>
        <button class="btn-publish" @click="goEdit">Agregar platos al menú</button>
      </div>

      <!-- Card del menú -->
      <div v-else-if="!loading" class="menu-center">
        <div class="menu-card">
          <div class="menu-card__header">
            <div>
              <h2 class="menu-card__title">Menú del local</h2>
          ...
          <div class="menu-card__dishes">
            <div v-for="dish in products.slice(0, 4)" :key="dish.id" class="dish-chip">
              <img :src="dish.imageUrl || placeholderImg" :alt="dish.name" class="dish-chip__img" />
              <div class="dish-chip__info">
                <span class="dish-chip__name">{{ dish.name }}</span>
                <span class="dish-chip__price">S/ {{ dish.price }}</span>
              </div>
              <span class="dish-chip__badge" :class="{ 'dish-chip__badge--off': !dish.available }">
                {{ dish.available ? 'Disponible' : 'No disponible' }}
              </span>
            </div>
          </div>

          <p v-if="products.length > 4" class="menu-card__more">
            + {{ products.length - 4 }} plato{{ products.length - 4 === 1 ? '' : 's' }} más
          </p>

          <div class="menu-card__footer">
            <span class="menu-card__count">{{ products.length }} plato{{ products.length === 1 ? '' : 's' }}</span>
            <span class="menu-card__available">{{ products.filter(p => p.available).length }} disponibles</span>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    async loadMenu() {
      ...
      this.errorMsg = ''
      try {
        const { exists, huariqueId, products } = await menuService.getMyMenu()
        this.huariqueId = huariqueId
        this.products = exists ? products : []
      } catch (e) {
    ...
    goEdit() {
      this.$router.push('/mi-menu/editar/me')
    }
  }
}
</script>
...
.btn-new-menu { display: flex; align-items: center; gap: 8px; padding: 12px 20px; background: var(--color-brand); color: #fff; border: none; border-radius: var(--radius-md); font-size: 0.88rem; font-weight: 700; font-family: var(--font-main); cursor: pointer; transition: background 0.2s; }
...
.menu-card { background: var(--color-card-bg); border-radius: var(--radius-xl); box-shadow: 0 2px 12px var(--color-card-shadow); padding: 1.5rem; display: flex; flex-direction: column; width: 100%; max-width: 480px; }
...
.dish-chip__info { display: flex; flex-direction: column; min-width: 0; flex: 1; }
...
.dish-chip__badge { font-size: 0.7rem; font-weight: 700; padding: 3px 8px; border-radius: var(--radius-pill); background: #dcfce7; color: #15803d; white-space: nowrap; }
.dish-chip__badge--off { background: #f3f4f6; color: var(--color-input-icon); }
...
.menu-card__available { font-size: 0.75rem; color: #15803d; font-weight: 600; }
